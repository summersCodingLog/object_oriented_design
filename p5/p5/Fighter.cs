...
namespace P5
{
    public class Fighter
    {
        ...
        private const double TargetDecreaser = 0.9;
        private const double ThresholdDecreaser = 0.2;
        ...
        public Fighter(int[] w)
        {
            ...
            if (w == null || w.Length == 0)
            {
                ...
                ArtilleryThreshold = Convert.ToInt32(Math.Floor(Artillery[w.Length - 1] * ThresholdDecreaser));
                ...
                StrengthThreshold = Convert.ToInt32(Strength * ThresholdDecreaser);
        ...
        public virtual bool IsActive()
        {
            ...
            {
                if (Artillery[i] > 1) { ActiveCount++; }
            }
            return ActiveCount > ArtilleryThreshold;
        }
        ...
        public bool Target(int x, int y, int q)
        {
            if (Armed)
            {
                if (RowAttkRange.Contains(x) && ColAttkRange.Contains(y) && Strength > q)
                { 
                    Strength = Convert.ToInt32(Math.Floor(Strength * TargetDecreaser));
                    KillCount++;
                    return true;
                }
            } 
